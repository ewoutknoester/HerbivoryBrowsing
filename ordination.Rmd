...
# When ReefType included, not significant (df =  1, SumOfSqs = 0.2748, F = 1.6675, p = 0.13889)
## ordid <- add_column(ordid, ReefType = "", .after = 1)
## ordid$ReefType <- ifelse(as.numeric(ordid$Location) < 3, "Patch", ifelse(as.numeric(ordid$Location) > 5, "Patch", "Fringe"))
## ordid$ReefType <- as.factor(ordid$ReefType)
